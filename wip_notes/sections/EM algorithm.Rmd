...
curve(w[1] * dnorm(x, mu[1], sigma[1]) + w[2] * dnorm(x, mu[2], sigma[2]), add = TRUE, col = 4, xlim = range(y))
```
...
calculate.loglik = function(y, mu, sigma, w, tau, K){
    sum(sapply(1:K, function(k) tau[,k] * (log(w[k]) + dnorm(y, mu[k], sigma[k], log = TRUE))))
}

predict.Z = function(tau) apply(tau, 1, function(x) which(x < (1/ncol(tau))))

# EM function
EM = function(y, mu, sigma, w, threshold = 1e-5){
    ...
    while(continue){
        ...
        # terminate
        difference = abs(calculate.loglik(y, mu0, sigma0, w0, tau, K) - calculate.loglik(y, mu, sigma, w, tau, K))
...
results$mu
results$sigma
hist(y[results$z == 1], xlim = range(y), col = 2, breaks = breaks, xlab = "y", main = "EM results")
